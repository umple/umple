...
private static void postpareTrace(UmpleModel aModel)
  ...
  private static void processTraceDirectiveStateMachines( TraceDirective traceDirective, CodeTranslator t, String template, String tracer) 
  {
	  ...
	  for( StateMachine_TraceItem tracedStm : traceDirective.getStateMachineTraceItems() )
	  {
		  ...
		  {
			  processTracedTransition(traceDirective,tracedStm.getTransition(),t,template);
		  }
		  
		  if( tracedStm.getEntry() )
		  {
			  processTracedStateEntry(traceDirective,tracedStm,t,template);
		  }
		  
		  if( tracedStm.getExit() )
		  {
			  processTracedStateExit(traceDirective,tracedStm,t,template);
		  }
		  
		  if(tracedStm.getTraceStateMachineFlag() )
		  {
			  processTracedStateMachine(traceDirective,tracedStm,t,template);
		  }
  ...
  private static void processTracedStateMachine(TraceDirective traceDirective, StateMachine_TraceItem tracedStm, CodeTranslator t, String template) 
  {
	  StateMachine stm = tracedStm.getStateMachine();
	  String stmCode = null;
	  stmCode = StringFormatter.format(template,prepareConsistentOutput("state",t.translate("stateMachineOne",stm)));
  ...
  private static void processTracedStateEntry(TraceDirective traceDirective, StateMachine_TraceItem tracedStm, CodeTranslator t, String template) 
  {
	  StateMachine stm = tracedStm.getStateMachine();
	  String stmCode = null;
	  stmCode = "if( " + t.translate("parameterOne",stm) + ".equals(" + t.translate("type",stm) + "." + stm.getState(0).getName() + ") )\n      ";
  ...
  private static void processTracedStateExit(TraceDirective traceDirective, StateMachine_TraceItem tracedStm, CodeTranslator t, String template) 
  {
	  StateMachine stm = tracedStm.getStateMachine();
	  String stmCode = null;
  ...
  private static void processTracedTransition( TraceDirective traceDirective, Transition tran, CodeTranslator t, String template) 
  {
	  String stmCode = null;
