...
private String getExtendClassesNames(UmpleClass uClass)
	  ...
	  for( int i = 0 ; i < traceDirective.numberOfStateMachineTraceItems() ; ++i )
	  {
		  ...
		  if(tracer.equals("console") && traceRecord != null )
		  {
			  ...
			  stmCode = "if( " + t.translate("stateMachineOne",stm) + ".equals(" + t.translate("type",stm) + "." + stm.getState(0).getName() + ") )\n      ";
	  		  stmCode += "print(\"action=" + stm.getState(0).getName() + "-exit" + ", " + traceRecord.getRecord() +  "=$\" + "+traceRecord.getRecord()+");";
  ...
  private void prepareFinalStateFor(StateMachine sm, StateMachine parentSm)
  {
    ...
    {
      deleteActionCode = StringFormatter.format("if ($this->{1}()) { $this->{0}(); }",translate("deleteMethod",sm.getUmpleClass()),translate("isFinalMethod",parentSm));
    }
    else
    {
