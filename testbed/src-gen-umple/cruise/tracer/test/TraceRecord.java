...
public class TraceRecord extends TraceStm
{
  ...
  public boolean setNumber1(int aNumber1)
  {
    ...
    ConsoleTracer.handle( System.currentTimeMillis()+","+Thread.currentThread().getId()+",TestHarnessTracer.ump,107,TraceRecord,"+System.identityHashCode(this)+",at_s,number1,"+number1+","+(""+aNumber1).toString().replace("\n","").replace(",",";")+",TracingNumber1" );
    number1 = aNumber1;
    wasSet = true;
    return wasSet;
  }
  ...
  {
    boolean wasSet = false;
    ConsoleTracer.handle( System.currentTimeMillis()+","+Thread.currentThread().getId()+",TestHarnessTracer.ump,108,TraceRecord,"+System.identityHashCode(this)+",at_s,number2,"+number2+","+(""+aNumber2).toString().replace("\n","").replace(",",";")+","+str );
    number2 = aNumber2;
    wasSet = true;
    return wasSet;
  }
