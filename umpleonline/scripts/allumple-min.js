...
}else{if(d=="StartOver"){Action.startOver()}else{if(d=="PngImage"){Action.pngImage()}else{if(d=="YumlImage"){Action.yumlImage()}else{if(d=="Jsf"){Action.uigu()
}else{if(d=="Copy"){Action.showCodeInSeparateWindow()}else{if(d=="CopyEncodedURL"){Action.showEncodedURLCodeInSeparateWindow()}else{if(d=="CopyLocalBrowser"){if(typeof(Storage)!=="undefined"){localStorage.setItem("umpleLocalStorage1",Page.getUmpleCode())
}}else{if(d=="LoadLocalBrowser"){if(typeof(Storage)!=="undefined"){var a=localStorage.getItem("umpleLocalStorage1");if(a!=null){Page.setUmpleCode(a)
}}}else{if(d=="Undo"){Action.undo()}else{if(d=="Redo"){Action.redo()}else{if(d=="ShowHideTextEditor"){Layout.showHideTextEditor()}else{if(d=="ShowHideCanvas"){Layout.showHideCanvas()
}else{if(d=="ShowEditableClassDiagram"){Action.changeDiagramType({type:"editableClass"})}else{if(d=="ShowJointJSClassDiagram"){Action.changeDiagramType({type:"JointJSClass"})
}else{if(d=="ShowGvClassDiagram"){Action.changeDiagramType({type:"GVClass"})}else{if(d=="ShowGvStateDiagram"){Action.changeDiagramType({type:"GVState"})
...
}else{if(d=="ManualSync"){Action.enableManualSync()}else{if(d=="SyncDiagram"){Action.processTyping("umpleModelEditorText",true)}else{if(d=="PhotoReady"){Action.photoReady()
}else{if(d=="ToggleAttributes"){Action.toggleAttributes()}else{if(d=="ToggleMethods"){Action.toggleMethods()}else{if(d=="ToggleActions"){Action.toggleActions()
}else{if(d=="ToggleTraits"){Action.toggleTraits()}else{if(d=="ToggleTransitionLabels"){Action.toggleTransitionLabels()}else{if(d=="ToggleGuardLabels"){Action.toggleGuardLabels()
...
}}}}}if(Page.useGvStateDiagram){if(!Page.showActions){a=a+".hideactions"}if(Page.showTransitionLabels){a=a+".showtransitionlabels"}if(Page.showGuardLabels){a=a+".showguardlabels"
}a=a+"."+$("inputGenerateCode").value.split(":")[1]}if(Page.useGvClassDiagram){if(Page.showMethods){a=a+".showmethods"}if(!Page.showAttributes){a=a+".hideattributes"
}}return a};function showTab(a){jQuery("#innerGeneratedCodeRow").nextAll().hide();jQuery("#innerGeneratedCodeRow"+a.data.tabnumber).show();
Page.showGeneratedCode(a.data.code,$("inputGenerateCode").value.split(":")[0],a.data.tabnumber);jQuery(".line").last().hide();jQuery(".line").last().hide();
jQuery("#innerGeneratedCodeRow").hide()}DiagramEdit=new Object();DiagramEdit.textChangeQueue=[];DiagramEdit.pendingChanges=false;DiagramEdit.newClass=null;
DiagramEdit.newAssociation=null;DiagramEdit.newGeneralization=null;DiagramEdit.updateUmpleText=function(a){if(DiagramEdit.textChangeQueue.length==0&&!DiagramEdit.pendingChanges){DiagramEdit.pendingChanges=true;
...
UmpleSystem.updateClass(a);UmpleSystem.redrawGeneralizationsTo(a);UmpleSystem.trimOverlappingAssociations(a);UmpleSystem.update()}};DiagramEdit.classDeleted=function(d){var a=true;
...
UmpleSystem.updateClass(a);UmpleSystem.redrawGeneralizationsTo(a);UmpleSystem.trimOverlappingAssociations(a);UmpleSystem.update()}};DiagramEdit.methodChanged=function(c,e,f,b){if(!Action.validateMethodName(b)){Action.updateUmpleDiagram();
setTimeout(function(){Page.setFeedbackMessage("Invalid UML Method. &lt;"+(b.split("&").join("&amp;").split("<").join("&lt;").split(">").join("&gt;"))+"&gt is not valid.")
...
};DiagramEdit.attributeDelete=function(b,d){var a=UmpleSystem.find(b);a.removeAttribute(d);var e=jQuery("#"+a.id);a.position.width=UmpleClassFactory.defaultSize.width;
var c=Json.toString(a);Page.showModelLoading();DiagramEdit.updateUmpleText({actionCode:format("action=editClass&actionCode={0}",c),codeChange:true});
...
Page.initHighlighter("buttonCopy");Page.initHighlighter("buttonCopyEncodedURL");Page.initHighlighter("buttonCopyLocalBrowser");Page.initHighlighter("buttonLoadLocalBrowser");
Page.initHighlighter("buttonSmaller");Page.initHighlighter("buttonLarger");Page.initHighlighter("buttonSyncCode");Page.initHighlighter("buttonSyncDiagram");
Page.initHighlighter("buttonToggleMethods");Page.initHighlighter("buttonToggleAttributes");Page.initHighlighter("buttonToggleActions");
Page.initHighlighter("buttonToggleTransitionLabels");Page.initHighlighter("buttonToggleGuardLabels");Page.initHighlighter("buttonToggleTraits");
Page.initToggleTool("buttonAddClass");Page.initToggleTool("buttonAddAssociation");Page.initToggleTool("buttonAddGeneralization");Page.initToggleTool("buttonDeleteEntity");
Page.initAction("buttonPngImage");Page.initAction("buttonYumlImage");Page.initAction("buttonPhpCode");Page.initAction("buttonEcoreCode");
Page.initAction("buttonJavaCode");Page.initAction("buttonJavaAPIDoc");Page.initAction("buttonRubyCode");Page.initAction("buttonAlloyCode");
Page.initAction("buttonNuSMVCode");Page.initAction("buttonSqlCode");Page.initAction("buttonCppCode");Page.initAction("buttonPhotoReady");
Page.initAction("buttonSimulateCode");Page.initAction("buttonShowHideTextEditor");Page.initAction("buttonShowHideCanvas");Page.initAction("buttonShowEditableClassDiagram");
...
Page.initAction("buttonShowStructureDiagram");Page.initAction("buttonShowHideLayoutEditor");Page.initAction("buttonManualSync");Page.initAction("buttonCopy");
Page.initAction("buttonCopyEncodedURL");Page.initAction("buttonCopyLocalBrowser");Page.initAction("buttonLoadLocalBrowser");Page.initAction("buttonUndo");
...
}else{if(j.indexOf(e)==0){e=j.substring(e.length,j.length)}else{if(j.indexOf(d)!=-1){d=j.substring(d.length,j.length)}}}}}if(h!=null&&h.length!=0){jQuery("#restorecode").show();
...
Action.loadFile();if(b.indexOf("d")!=-1){if(!jQuery("#buttonShowHideCanvas").prop("checked")){jQuery("#buttonShowHideCanvas").trigger("click")
...
cookie_pref="PREF=";if(jQuery("#buttonPhotoReady").prop("checked")){cookie_pref+="p"}if(jQuery("#buttonManualSync").prop("checked")){cookie_pref+="s"
