...
Page.init = function (
	...
	if (Page.readOnly) {
		jQuery('#' + Page.umpleCanvasId()).addClass('photoReady');
	}
...
Page.initHighlighter = function (id) {
	...
	jQuery(selector).mouseover(function () {
		Page.highlightItem(id);
	});
	jQuery(selector).mouseout(function () {
		Page.unhighlightItem(id);
	});
};
...
Page.initToggleTool = function (id) {
	...
	item.unbind().click(function () {
		Page.toggleToolItem(id, false);
	});
	item.dblclick(function () {
		Page.toggleToolItem(id, true);
	});
};
...
Page.initLabels = function () {
	...
	for (var i = 0, len = labels.length; i < len; i++) {
		...
		jQuery(labelId).click(
			(function (x) {
				return function () {
					jQuery('#' + jQuery(x).prop('id').replace('label', 'button')).trigger(
						'click'
					);
				};
			})(labelId)
...
Page.initUmpleTextArea = function () {
	...
	modelEditor.mousedown(function () {
		setTimeout('jQuery("#linenum").val(Action.getCaretPosition())', 25);
	});
	...
	modelEditor.focus(function () {
		Action.focusOn('umpleModelEditorText', true);
	});
	layoutEditor.focus(function () {
		Action.focusOn('umpleLayoutEditorText', true);
	});
	modelEditor.blur(function () {
		Action.focusOn('umpleModelEditorText', false);
	});
	layoutEditor.blur(function () {
		Action.focusOn('umpleLayoutEditorText', false);
	});

	Page.initCodeMirrorEditor();
	Layout.initUmpleTextAreaSize();
	if (!Layout.isTextVisible) {
		Layout.showHideTextEditor(false);
	}
	if (!Layout.isLayoutVisible) {
		Layout.showHideLayoutEditor(false);
	}
};
...
Page.initCodeMirrorEditor = function () {
	...
	Page.codeMirrorEditor = CodeMirror.fromTextArea(
		document.getElementById('umpleModelEditorText'),
		{
			...
			extraKeys: {
				// Change consistently in umple_action.js for Mousetrap
				'Ctrl-E': function (cm) {
...
Page.initCanvasArea = function () {
	...
	canvas.click(function (event) {
		Action.umpleCanvasClicked(event);
	});
	canvas.mousemove(Action.mouseMove);
	canvas.focus(function () {
		Action.focusOn(Page.umpleCanvasId(), true);
	});
	canvas.blur(function () {
		Action.focusOn(Page.umpleCanvasId(), false);
	});
	canvas.delegate(
		"[class$='editableDoubleClick']",
		'dblclick',
		InlineEditor.handleOnClick
	);
	canvas.delegate(
		"[class$='editableSingleClick']",
		'click',
		InlineEditor.handleOnClick
	);

	if (!Layout.isDiagramVisible) {
		Layout.showHideCanvas(false);
	}
};
...
Page.setExamples = function (ex) {
	...
	jQuery('#inputExample option').each(function () {
		if (this.value == exValue) {
			jQuery('#inputExample option[value = "' + exValue + '"]').attr(
				'selected',
				true
			);
			return;
	...
	jQuery('#inputExample2 option').each(function () {
		if (this.value == exValue) {
			jQuery('#inputExample2 option[value = "' + exValue + '"]').attr(
				'selected',
				true
			);
			return;
	...
	jQuery('#inputExample3 option').each(function () {
		if (this.value == exValue) {
			jQuery('#inputExample3 option[value = "' + exValue + '"]').attr(
				'selected',
				true
			);
			return;
	...
	jQuery('#inputExample4 option').each(function () {
		if (this.value == exValue) {
			jQuery('#inputExample4 option[value = "' + exValue + '"]').attr(
				'selected',
				true
			);
			return;
...
Page.getUmpleCode = function () {
	...
	var positioning = jQuery('#umpleLayoutEditorText')
		.val()
		.replace(Page.modelDelimiter, '');
...
Page.createTask = function () {
	...
	if (!patt.test(taskName)) {
		//taskName.indexOf(" ") != -1 || taskName.indexOf("/") != -1 || taskName.indexOf("-") != -1 || taskName.indexOf("\\") != -1)
		window.alert(
			'Task Name can only contain letters(case insensitive), underscores, dots, and digits!'
		);
		return;
	}
	...
	Ajax.sendRequest(
		'task.php',
		Page.createTaskCallback,
		format(
			'taskName={0}&instructions={1}&model={2}&requestorName={3}&completionURL={4}&isExperiment={5}',
			taskName,
			instructions.val(),
			Page.getModel(),
			requestorName,
...
Page.createTaskCallback = function (response) {
	if (response.responseText.split(' ')[0] == 'Task') {
		window.alert(
			'Not able to create a task with that name. ' + response.responseText
		);
	} else {
...
Page.cancelTaskResponse = function () {
	...
	if (answer) {
		window.location.href =
			'task.php?cancelTaskResponse=1&model=' + Page.getModel();
	}
};
...
Page.cancelTask = function () {
	if (
		...
		var answer = confirm(
			'Are you sure you want to end the task creation process?'
		);
...
Page.endTaskEdit = function () {
	var answer = confirm(
		'Are you sure you want to end the task editing process?'
	);
	if (answer) {
		Page.hideTask();
	}
...
Page.showDiagramSyncNeeded = function (doShow) {
	var canvas = jQuery('#umpleCanvas');
	var messageDiv =
		'<div id="syncNeededMessage" class="syncNeededMessage unselectable">' +
...
Page.resetCanvasSize = function () {
	Page.setUmpleCanvasSize(
		Layout.minCanvasSize.width,
		Layout.minCanvasSize.height
	);
};
...
Page.applyGeneratedCodeAreaStyles = function (language) {
	...
	//One of the svg diagram types
		...
		language == 'stateDiagram' ||
		language == 'classDiagram' ||
		language == 'structureDiagram'
	) {
		generatedArea.removeClass('generatedCode');
...
Page.toggleStructureDiagramLink = function (isGenerated, filename) {
	...
	if (isGenerated) {
		linkContainer.html(
			format(
				"<a href='{0}' target='_blank' id='structureLink'>Download the SVG for the following</a>",
				filename
			)
		);
		...
		linkContainer.html(
			'<div id="buttonStructureLink" value="Generate SVG"></div>'
		);
