  ...
  if (inputType == "rename") {
    ...
    var inputFunc = function setter(e) {
      if (e.key === 'Enter') {
        ...
        Action.setColorState(stateCode, stateName, input.value);
        document.removeEventListener("mousedown", hider);
        prompt.remove();
        Action.removeContextMenu();
...
Action.drawStateMenu = function (event) {
  ...
  // Grab stateClicked payload robustly (handles &quot; / " / ')
  var html = (targ && targ.outerHTML) ? targ.outerHTML : "";
  var m = html.match(/stateClicked\((?:&quot;|"|')([^"']+?)(?:&quot;|"|')\)/);
  if (!m || !m[1]) {
    return;
  }
  var elemText = m[1].split("^*^"); // [0]=class, [1]=base stateMachine, [2]=state path
  if (!elemText || elemText.length < 3 || typeof elemText[2] !== "string") {
    return;
  }
  ...
  var rowFuncs = ["Action.drawInputState(\"rename\",\"" + jsInput + "\",\"" + elemText[2][elemText[2].length - 1] + "\")", "Action.deleteState(\"" + jsInput + "\",\"" + elemText[0] + "\",\"" + elemText[1] + "\",\"" + elemText[2] + "\")", "Action.drawInputState(\"substate\",\"" + jsInput + "\",\"" + elemText[2][elemText[2].length - 1] + "\")", "Action.drawInputState(\"transition\",\"" + jsInput + "\",\"" + elemText[2][elemText[2].length - 1] + "\")", "Action.drawInputState(\"colorState\",\"" + jsInput + "\",\"" + elemText[2][elemText[2].length - 1] + "\")"];
...
Action.setColorState = function(stateCode, stateName, color){
  var orig = Page.codeMirrorEditor6.state.doc.toString();
  ...
  var newCode = stateCode;
  if(newCode.indexOf("displayColor") >= 0){
    newCode = newCode.replace(/displayColor\s+#[0-9a-fA-F]{6}\s*;/, "displayColor "+color+";");
  } else {
    var braceIndex = newCode.indexOf("{");
    if(braceIndex >= 0){
      newCode = newCode.slice(0, braceIndex+1) + "\n  displayColor " + color + ";\n" + newCode.slice(braceIndex+1);
    }
  }


  Page.codeMirrorEditor6.dispatch({
    changes: {from: startIndex, to: endIndex, insert: newCode}
  });
  Page.reindent();
}
...
Action.setCaretPosition = function (line) {
  if (isNaN(line - 0)) {
    ...
    if (line == "sp") { // creates Survey Pass; modifies conditions to allow for survey to be displayed:
    ...
    if (line == "sc") { // clears all survey cookies including whether URL has been shown already, whether the user has been skipped, and whether Survey Pass has been activated
    ...
    if (line == "tc") { // resets cookies for tips
