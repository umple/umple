...
class MentorF
  ...
  #MentorF Associations - for documentation purposes
  ...
  def initialize(all_students)
    ...
    raise "Unable to create MentorF, must have at least 2 @students" unless did_add_students 
    @program = nil
    @initialized = true
  end
  ...
  def add_student(a_student)
    ...
    if !existing_mentor.nil? and existing_mentor.number_of_students <= MentorF.minimum_number_of_students
      return was_added
  ...
  def remove_student(a_student)
    was_removed = false
    if @students.include?(a_student) and number_of_students > MentorF.minimum_number_of_students
  ...
  def set_students(new_students)
    was_set = false
    if new_students.length < MentorF.minimum_number_of_students
      return was_set
    end
