...
class ClassMStarAssociation
  ...
  #ClassMStarAssociation Associations - for documentation purposes
  ...
  def initialize(all_otherclasses)
    ...
    raise "Unable to create ClassMStarAssociation, must have at least 2 @otherclasses" unless did_add_otherclasses 
    @initialized = true
  end
  ...
  def set_otherclasses(new_otherclasses)
    ...
    if verified_otherclasses.size != new_otherclasses.size or verified_otherclasses.size() < ClassMStarAssociation.minimum_number_of_otherclasses
      return was_set
    end
