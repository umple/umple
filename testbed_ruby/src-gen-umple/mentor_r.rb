...
class MentorR
  ...
  #MentorR Attributes - for documentation purposes
  #attr_reader :name

  #MentorR Associations - for documentation purposes
  ...
  def initialize(a_name, all_students)
    ...
    raise "Unable to create MentorR, must have at least 3 @students" unless did_add_students 
    @program = nil
    @initialized = true
  end
  ...
  def is_number_of_students_valid
    is_valid = number_of_students >= MentorR.minimum_number_of_students
    is_valid
  end
  ...
  def remove_student(a_student)
    ...
    if number_of_students <= MentorR.minimum_number_of_students
      return was_removed
    end
  ...
  def set_students(new_students)
    ...
    if verified_students.size != new_students.size or verified_students.size < MentorR.minimum_number_of_students
      return was_set
    end
