...
class MentorAL
  ...
  #MentorAL Attributes - for documentation purposes
  #attr_reader :name

  #MentorAL Associations - for documentation purposes
  ...
  def initialize(a_name, all_students)
    ...
    raise "Unable to create MentorAL, must have at least 3 @students" unless did_add_students 
    @program = nil
    @initialized = true
  end
  ...
  def remove_student(a_student)
    ...
    if number_of_students <= MentorAL.minimum_number_of_students
      return was_removed
    end
  ...
  def set_students(new_students)
    ...
    if verified_students.size != new_students.size or verified_students.size() < MentorAL.minimum_number_of_students
      return was_set
    end
