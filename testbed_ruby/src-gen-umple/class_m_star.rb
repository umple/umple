...
class ClassMStar
  ...
  #ClassMStar Associations - for documentation purposes
  ...
  def initialize(all_classOtherclasses)
    ...
    raise "Unable to create ClassMStar, must have at least 2 @classOtherclasses" unless did_add_classOtherclasses 
    @initialized = true
  end
  ...
  def set_classOtherclasses(new_classOtherclasses)
    ...
    if verified_classOtherclasses.size != new_classOtherclasses.size or verified_classOtherclasses.size() < ClassMStar.minimum_number_of_classOtherclasses
      return was_set
    end
