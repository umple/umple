...
class ClassN
  ...
  #ClassN Associations - for documentation purposes
  ...
  def initialize(all_classOtherclasses)
    ...
    raise "Unable to create ClassN, must have 2 @classOtherclasses" unless did_add_classOtherclasses 
    @initialized = true
  end
  ...
  def self.required_number_of_classOtherclasses
    2
  end
  ...
  def set_classOtherclasses(new_classOtherclasses)
    ...
    if verified_classOtherclasses.size != new_classOtherclasses.size or verified_classOtherclasses.size() != ClassN.required_number_of_classOtherclasses
      return was_set
    end
