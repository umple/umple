...
class ClassOptionalNAssociation
  ...
  #ClassOptionalNAssociation Associations - for documentation purposes
  ...
  def initialize(all_otherclasses)
    ...
    raise "Unable to create ClassOptionalNAssociation, must have 2 or fewer @otherclasses, no duplicates" unless did_add_otherclasses
    @initialized = true
  end
  ...
  def set_otherclasses(new_otherclasses)
    ...
    if verified_otherclasses.size != new_otherclasses.size or verified_otherclasses.size() > ClassOptionalNAssociation.maximum_number_of_otherclasses
      return was_set
    end
