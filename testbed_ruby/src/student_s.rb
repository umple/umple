...
class StudentS
  ...
  #StudentS Attributes - for documentation purposes
  #attr_reader :number

  #StudentS Associations - for documentation purposes
  ...
  def is_number_of_mentors_valid
    is_valid = number_of_mentors >= StudentS.minimum_number_of_mentors and number_of_mentors <= StudentS.maximum_number_of_mentors
    is_valid
  end
  ...
  def add_mentor(a_mentor)
    ...
    if number_of_mentors >= StudentS.maximum_number_of_mentors
      return was_added
    end
  ...
  def remove_mentor(a_mentor)
    ...
    if number_of_mentors <= StudentS.minimum_number_of_mentors
      return was_removed
    end
  ...
  def set_mentors(new_mentors)
    ...
    if verified_mentors.size != new_mentors.length or verified_mentors.size < StudentS.minimum_number_of_mentors or verified_mentors.size() > StudentS.maximum_number_of_mentors
      return was_set
    end
  ...
  def delete
    ...
    copy_of_mentors.each do |a_mentor|
      if a_mentor.number_of_students <= MentorS.minimum_number_of_students
        a_mentor.delete
      else
