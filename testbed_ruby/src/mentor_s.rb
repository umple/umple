...
class MentorS
  ...
  #MentorS Attributes - for documentation purposes
  #attr_reader :name

  #MentorS Associations - for documentation purposes
  ...
  def is_number_of_students_valid
    is_valid = number_of_students >= MentorS.minimum_number_of_students and number_of_students <= MentorS.maximum_number_of_students
    is_valid
  end
  ...
  def add_student(a_student)
    ...
    if number_of_students >= MentorS.maximum_number_of_students
      return was_added
    end
  ...
  def remove_student(a_student)
    ...
    if number_of_students <= MentorS.minimum_number_of_students
      return was_removed
    end
  ...
  def set_students(new_students)
    ...
    if verified_students.size != new_students.length or verified_students.size < MentorS.minimum_number_of_students or verified_students.size() > MentorS.maximum_number_of_students
      return was_set
    end
  ...
  def delete
    ...
    copy_of_students.each do |a_student|
      if a_student.number_of_mentors <= StudentS.minimum_number_of_mentors
        a_student.delete
      else
